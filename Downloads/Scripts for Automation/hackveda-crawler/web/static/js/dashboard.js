// HackVeda Crawler Dashboard JavaScript
...
async function checkSystemHealth() {
    try {
        ...
        if (data.status === 'healthy') {
            // Only update to healthy if socket is also connected
            if (isConnected) {
...
async function startCrawling() {
    ...
    try {
        ...
        showToast('error', 'Crawling Failed', `${error.message} - Check console for details`);
        resetCrawlUI();
    }
    
    } catch (outerError) {
        console.error('❌ Critical error in startCrawling:', outerError);
        console.error('❌ Error stack:', outerError.stack);
        showToast('error', 'Critical Error', `${outerError.message} - Check console`);
    }
}
...
function updateStatsCards(stats) {
    document.getElementById('total-sessions').textContent = stats['Crawl Sessions'] || 0;
    document.getElementById('total-results').textContent = stats['Search Results'] || 0;
    document.getElementById('emails-sent').textContent = stats['Email Logs'] || 0;
    document.getElementById('total-domains').textContent = stats['Domains'] || 0;
}
